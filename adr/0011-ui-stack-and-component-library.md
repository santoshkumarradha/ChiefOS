...
| Native material (liquid glass) | **`tauri-plugin-window-vibrancy`** on macOS → `NSVisualEffectView`. On Linux/Wayland: `layer-shell` + compositor blur. CSS `backdrop-filter` is the fallback, not the primary. | Real liquid glass, not a web approximation. |
| Component primitives (unstyled + accessible) | **Radix UI** (React) | Headless, WAI-ARIA-complete. We style them with our own tokens. No opinionated theme to fight. Shadcn is NOT used (too Tailwind-opinionated; smells web). |
