//! End-to-end live pack install test for the Platform MVP demo.
//!
//! This boots real `AppState`, previews an on-disk pack manifest through HTTP,
//! then activates the new pack against the existing Work Object through the
//! public `chief-sdk` context.
...
#[tokio::test]
async fn installed_risk_pack_contributes_to_existing_work_object() {
    assert_no_risk_pack_coupling("packs/document-pack/src/lib.rs");
    assert_no_risk_pack_coupling("packs/calendar-pack/src/lib.rs");
    assert_no_risk_pack_coupling("packs/email-pack/src/lib.rs");
    assert_no_risk_pack_coupling("apps/chief-brief-ui/src/surfaces/WorkObjectView.tsx");
    ...
    let before = get_work(&srv).await;
    assert_eq!(count_kind_from_pack(&before, "risk", "risk-pack"), 0);

    let manifest_path = workspace_root()
        .join("packs/risk-pack/manifest.toml")
        .canonicalize()
        .expect("risk manifest path");
    let preview: Value = reqwest::Client::new()
        .post(srv.url("/v1/packs/install-preview"))
        .json(&json!({ "manifest_path": manifest_path }))
        .send()
        .await
    ...
    assert_eq!(preview["pack_id"], "risk-pack");
    assert_eq!(preview["signature_status"], "placeholder_verified");
    assert_eq!(preview["installable"], true);
    let grant_kinds: HashSet<_> = preview["requested_grants"]
        .as_array()
        .expect("requested grants")
        .iter()
        .filter_map(|grant| grant["kind"].as_str())
        .collect();
    assert!(grant_kinds.contains("mem.read"));
    assert!(grant_kinds.contains("mem.write"));
    ...
    let after = get_work(&srv).await;
    assert_eq!(count_kind_from_pack(&after, "risk", "risk-pack"), 1);
}

async fn get_work(srv: &TestServer) -> Value {
    reqwest::get(srv.url("/v1/work/acme-follow-up"))
        .await
        .expect("GET work")
        .json()
        .await
...
fn count_kind_from_pack(work: &Value, kind: &str, pack: &str) -> usize {
    work["contributions"]
        .as_array()
        .expect("contributions")
        .iter()
        .filter(|node| node["kind"] == kind && node["pack"] == pack)
        .count()
}
...
async fn seed_acme_work_object(state: &AppState) -> String {
    ...
    let work_uri = mem
        .put_node(Node::new(
            NodeType::Artifact,
            Horizon::Medium,
            "chief-core:live-pack-install-e2e",
            work_body,
        ))
    ...
    for source in [&contract_uri, &calendar_uri, &prior_email_uri] {
        mem.put_edge(Edge {
            ...
            weight: 1.0,
            created_by: "chief-core:live-pack-install-e2e".into(),
        })
...
fn assert_no_risk_pack_coupling(path: &str) {
    let content = std::fs::read_to_string(workspace_root().join(path)).expect("read source");
    assert!(
        !content.contains("risk_pack") && !content.contains("risk-pack"),
        "{path} must not special-case risk-pack"
    );
}
