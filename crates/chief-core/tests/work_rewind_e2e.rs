//! End-to-end Work Object provenance and rewind test.
...
#[tokio::test]
async fn rewind_removes_active_draft_but_preserves_provenance() {
    ...
    run_base_packs(Arc::clone(&state)).await;

    let app = router_with_dist(Arc::clone(&state), None);
    let srv = spawn(app).await;
    let before = get_work(&srv).await;
    let draft_uri = contribution_uri(&before, "draft_reply").expect("draft contribution");

    seed_downstream_artifact(&state, &draft_uri).await;

    let response: Value = reqwest::Client::new()
        .post(srv.url("/v1/work/acme-follow-up/rewind"))
        .json(&json!({
            "contribution_uri": draft_uri,
            "reason": "draft needs customer-safe rewrite"
        }))
        .send()
        .await
        .expect("rewind")
    ...
    assert_eq!(response["stale_marked"], 1);

    let after = get_work(&srv).await;
    assert_eq!(count_kind(&after, "draft_reply"), 0);
    assert_eq!(count_kind(&after, "send_plan"), 1);

    let provenance: Value = reqwest::get(srv.url("/v1/work/acme-follow-up/provenance"))
        .await
        .expect("provenance")
        .json()
        .await
        .expect("provenance json");
    let rows = provenance.as_array().expect("provenance rows");
    assert!(rows.iter().any(|row| {
        row["kind"] == "rewind_event" && row["body"]["target_body"]["kind"] == "draft_reply"
    }));
    assert!(rows.iter().any(|row| row["kind"] == "stale_marker"));
}

async fn run_base_packs(state: Arc<AppState>) {
    DocumentAgent
        .on_tick(ctx_for(
            Arc::clone(&state),
            &["artifact", "file"],
...
fn contribution_uri(work: &Value, kind: &str) -> Option<String> {
    work["contributions"]
        .as_array()?
        .iter()
...
fn count_kind(work: &Value, kind: &str) -> usize {
    work["contributions"]
        .as_array()
...
async fn seed_acme_work_object(state: &AppState) -> String {
    ...
    for source in [&contract_uri, &calendar_uri, &prior_email_uri] {
        mem.put_edge(Edge {
            ...
            weight: 1.0,
            created_by: "chief-core:work-rewind-e2e".into(),
        })
...
async fn seed_downstream_artifact(state: &AppState, draft_uri: &str) {
    let mem = state.mem.lock().await;
    mem.put_node(Node::new(
        NodeType::Artifact,
        Horizon::Medium,
        "pack:email-pack/send-planner",
        json!({
            "kind": "send_plan",
            "work_object_id": "acme-follow-up",
            "content": "Send the drafted reply after Ceremony approval.",
            "source_refs": [draft_uri],
        })
        .to_string(),
    ))
    .expect("put downstream artifact");
}
